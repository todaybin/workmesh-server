...
func NewGroupService(root string) *GroupService {
	...
	db := SharedDatabase()
	s := &GroupService{legacyPath: filepath.Join(root, "groups.json")}
	if db == nil {
		s.initErr = errors.New("公共数据库未初始化")
		return s
	}
	s.repository, s.initErr = storage.NewSQLiteRepository(db)
	if s.initErr != nil {
		return s
	}
...
func (s *GroupService) initialize() error {
	...
	if _, err := s.repository.Exec(`CREATE TABLE IF NOT EXISTS resource_groups (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, type TEXT NOT NULL, is_default INTEGER NOT NULL DEFAULT 0, is_delete INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL, updated_at TEXT NOT NULL, UNIQUE(type,name))`); err != nil {
		return fmt.Errorf("初始化分组表失败: %w", err)
	}
	if _, err := s.repository.Exec(`CREATE INDEX IF NOT EXISTS idx_resource_groups_type_default ON resource_groups(type,is_default DESC,id)`); err != nil {
		return fmt.Errorf("初始化分组索引失败: %w", err)
	}
	var count int
	if err := s.repository.QueryRow(`SELECT COUNT(*) FROM resource_groups`).Scan(&count); err != nil {
		return err
	}
	if count == 0 {
		var legacy []Group
		if payload, err := os.ReadFile(s.legacyPath); err == nil && json.Unmarshal(payload, &legacy) == nil {
			for _, group := range legacy {
				if err := s.insertLegacy(group); err != nil {
					return err
				}
			}
		}
	...
	for _, kind := range []string{"website", "host"} {
		var exists int
		if err := s.repository.QueryRow(`SELECT COUNT(*) FROM resource_groups WHERE type=?`, kind).Scan(&exists); err != nil {
			return err
		}
		if exists == 0 {
			now := time.Now().UTC().Format(time.RFC3339Nano)
			if _, err := s.repository.Exec(`INSERT INTO resource_groups(name,type,is_default,is_delete,created_at,updated_at) VALUES('Default',?,1,0,?,?)`, kind, now, now); err != nil {
				return err
			}
		}
	}
...
func (s *GroupService) insertLegacy(group Group) error {
	if strings.TrimSpace(group.Name) == "" || strings.TrimSpace(group.Type) == "" {
		return errors.New("旧分组数据缺少名称或类型")
	}
	now := time.Now().UTC().Format(time.RFC3339Nano)
	_, err := s.repository.Exec(`INSERT OR IGNORE INTO resource_groups(id,name,type,is_default,is_delete,created_at,updated_at) VALUES(?,?,?,?,?,?,?)`, group.ID, group.Name, group.Type, databaseBoolInt(group.IsDefault), databaseBoolInt(group.IsDelete), now, now)
	return err
}
...
func (s *GroupService) List(kind string) []Group {
	...
	rows, err := s.repository.Query(`SELECT id,name,type,is_default,is_delete FROM resource_groups WHERE (?='' OR type=?) ORDER BY is_default DESC,id LIMIT 1000`, strings.TrimSpace(kind), strings.TrimSpace(kind))
	if err != nil {
		return []Group{}
	}
	defer rows.Close()
...
func (s *GroupService) Upsert(id uint, name, kind string, isDefault bool) (Group, error) {
	...
	err := s.repository.WithTx(context.Background(), func(tx storage.SQLExecutor) error {
		if isDefault {
...
func (s *GroupService) Delete(id uint, inUse func(uint) bool) error {
	...
	var isDefault int
	if err := s.repository.QueryRow(`SELECT is_default FROM resource_groups WHERE id=?`, id).Scan(&isDefault); err != nil {
	...
	_, err := s.repository.Exec(`DELETE FROM resource_groups WHERE id=?`, id)
	return err
}
