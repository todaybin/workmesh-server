...
const iptablesChains = ['WORKMESH_BASIC_BEFORE', 'WORKMESH_BASIC', 'WORKMESH_BASIC_AFTER'] as const;
...
const providerScopes = (): Firewall.Scope[] => {
    if (provider.value === 'iptables' || provider.value === 'nftables') {
        return (['ipv4', 'ipv6'] as Firewall.Family[]).flatMap((family) =>
            ['WORKMESH_BASIC_BEFORE', 'WORKMESH_BASIC', 'WORKMESH_BASIC_AFTER'].map((chain) => ({
...
const isIptablesSystemPresetScope = (scope: Firewall.Scope) =>
    ...
    (scope.chain === 'WORKMESH_BASIC_BEFORE' || scope.chain === 'WORKMESH_BASIC_AFTER');
...
const priorityPositionRanges = (
    ...
    const chain = item?.rule.scope.chain || 'WORKMESH_BASIC';
...
const exportRules = async (rows: RuleRow[]) => {
    ...
    downloadWithContent(JSON.stringify(exported, null, 2), `workmesh-firewall-rules-${getCurrentDateFormatted()}.json`);
};
...
const displayRulePriority = (row: Firewall.InventoryItem) => {
    ...
    if (
        ...
        row.rule.scope.chain !== 'WORKMESH_BASIC'
    )
        return '-';
