...
const getApp = async () => {
    ...
    try {
        ...
        const data = (res?.data || {}) as Partial<App.AppDTO>;
        const versions = Array.isArray(data.versions)
            ? data.versions.filter((item: unknown): item is string => typeof item === 'string' && item.trim() !== '')
            : [];
        app.value = { ...data, versions };
        version.value = versions[0] || '';
        if (version.value && Number.isFinite(Number(app.value.id))) {
            await getDetail(app.value.id, version.value);
        }
    } finally {
...
const getDetail = async (id: number, version: string) => {
    ...
    try {
        ...
        if (typeof appDetail.value.architectures === 'string' && appDetail.value.architectures !== '') {
