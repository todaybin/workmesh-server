...
const gatewayLoginForm = reactive({ username: '', password: '' });
...
const gatewayRegisterURL = computed(
    () => `${(gatewayStatus.gatewayUrl || 'https://work.zoomtk.com').replace(/\/$/, '')}/login/register`,
);
...
const search = async () => {
    ...
    try {
        Object.assign(gatewayStatus, (await getWorkMeshGatewayStatus()).data);
    } catch {
...
.gateway-binding-row :deep(.el-input) {
    flex: 1 1 180px;
    min-width: 0;
}
...
@media (max-width: 640px) {
    .gateway-binding-row :deep(.el-input),
    .gateway-binding-row :deep(.el-button) {
        width: 100%;
        flex-basis: 100%;
    }
}
