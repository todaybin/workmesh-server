...
const message = {
    ...
    database: {
        ...
        supplementPasswordHelper: '既存のパスワードを WorkMesh に保存するだけで、MySQL のパスワードは変更しません。',
        pgBindHelper:
            'この操作は、新しいユーザーを作成し、ターゲットデータベースにバインドするために使用されます。現在、データベースに既存のユーザーを選択することはサポートされていません。',
        pgSuperUser: 'スーパーユーザー',
        loadFromRemoteHelper: 'サーバー上のデータベース、ユーザー、権限情報を WorkMesh に同期します。続行しますか？',
    ...
    aiTools: {
        ...
        model: {
            ...
            from_remote: 'このモデルはWorkMeshを介してダウンロードされておらず、関連するプルログはありません。',
        ...
        benchmark: {
            ...
            tokenizerPlaceholder:
                'ローカルトークナイザーディレクトリを選択または入力してください。例: /opt/workmesh/tokenizers/DeepSeek-V3',
            runConfig: '実行設定',
            rawResult: '生データ',
            resultMetrics: '結果指標',
        ...
        mcp: {
            ...
            gatewayArgsHelper:
                '引数は supergateway の起動コマンドに追加されます。WorkMesh が管理する転送方式、ポート、パス、プロトコルの引数は変更できません',
    ...
    container: {
        ...
        containerDeleteHelper:
            'コンテナがアプリストアから作成されたことを検出しました。コンテナを削除しても、WorkMesh から完全には削除されません。完全に削除するには、アプリストアの「インストール済み」または「実行環境」などのメニューに移動して操作してください。続行しますか？',
        start: '始める',
        stop: '停止',
        restart: '再起動',
        kill: '殺す',
        ...
        composeEnvHelper2:
            'このオーケストレーションはWorkMeshアプリストアで作成されました。インストール済みアプリケーションで環境変数を変更してください。',
    ...
    file: {
        ...
        clashDidNotSupport: 'ファイル名は、.workmesh_clashを含むことを禁止されています',
        ...
        panelInstallDir: 'WorkMeshインストールディレクトリは削除できません',
        wgetTask: 'ダウンロードタスク',
    ...
    setting: {
        ...
        apiTrustedProxiesHelper:
            'リバースプロキシを使用する場合は、その IP またはネットワークを入力してください。空欄の場合はどのプロキシも信頼せず、直接の接続元 IP を使用します。0.0.0.0/0 と ::/0 は指定できません。',
        ...
        passkeyPrereqBindDomain: 'WorkMesh のドメインバインド設定済み',
        ...
        allowIPsPrivateHelper:
            'プライベートネットワークのアドレスも制限されます。ローカル接続には 127.0.0.1 または ::1 を追加し、リバースプロキシには信頼済みプロキシを設定してください。',
        ...
        rollbackLocalHelper:
            'マスターノードは直接ロールバックをサポートしていません。手動で「wh restore」コマンドを実行してロールバックしてください！',
    ...
    license: {
        ...
        node: '商用版にアップグレードすると、WorkMesh で複数の Linux サーバーを管理できます。',
        nodeApp:
        ...
        vllm: '商用版にアップグレードすると、WorkMesh で vLLM サービスのデプロイ、設定、ライフサイクル操作、タスク追跡を一元管理できます。',
        'model-downloader':
    ...
    ssl: {
        ...
        shell: 'スクリプトコンテンツ',
        shellHelper:
            'スクリプトのデフォルトの実行ディレクトリは、WorkMeshインストールディレクトリです。証明書がローカルディレクトリにプッシュされた場合、実行ディレクトリは証明書プッシュディレクトリになります。デフォルトの実行タイムアウトは30分です。',
    ...
    firewall: {
        ...
        unUsed: '未使用',
        managed: 'パネル作成',
        managedHelper: 'WorkMesh が作成・管理するルールです。編集または削除できます。',
        adopted: '外部ルール管理',
        adoptedHelper: 'システムに既に存在し、現在は WorkMesh が管理しているルールです。',
        external: '外部ルール',
        externalHelper:
            'システムに存在しますが、まだ WorkMesh の管理対象ではありません。必要に応じて管理対象にできます。',
        protected: 'システム保護',
        ...
        adoptRuleConfirm: '管理対象にすると、WorkMesh がこの既存ルールの保守と削除を行えるようになります。続行しますか？',
        ...
        plan_equivalent_managed_rule: '同じルールは既に WorkMesh の管理対象です。重複作成は不要です。',
        ...
        scopeUnmanagedActive: '他の有効な範囲が見つかりました：{0}。WorkMesh はそのルールを変更しません。',
        ...
        dockerFirewallHelper: 'WorkMesh のコンテナポート保護の管理方法を選択します。',
        ...
        resetDirectRulesHelper:
            '{0} から WorkMesh システムファイアウォールのチェーン、実行ルール、永続化ファイルを削除し、データベースポリシーは保持します',
        ...
        cleanupForwardingBackendHelper:
            '{0} の WorkMesh ポート転送ランタイムルールをリセットします。関連するルールとチェーンをすべて削除し、データベースデータを保持します',
        cleanupDockerBackendHelper:
            '{0} の WorkMesh Docker ポート保護ランタイムルールをリセットします。関連するルールとチェーンをすべて削除し、データベースデータを保持します',
        cleanupBeforeBackendSwitch:
            '現在の {0} バックエンドには WorkMesh の実行時ルールが残っています。{1} に切り替える前にリセットしてください。',
        cleanupAction: 'リセット',
        backendSwitchNotice:
        ...
        ruleSyncHelper:
            '選択したファイアウォールの WorkMesh 管理ルールを現在のファイアウォールへ同期します。元のルールは削除されず、外部ルールは同期されません。',
        ruleSyncDatabase: 'WorkMesh データベース',
        ruleSyncDatabaseHelper:
            'WorkMesh データベースのルールを基準に、現在のファイアウォールの管理対象ルールを同期・調整します。不足しているルールは追加され、余分なルールは削除されます。',
    ...
    tool: {
        supervisor: {
            ...
            numprocs: '番号。プロセスの',
            initWarn:
                'これにより、メイン構成ファイルの[[include] [include]セクションの「ファイル」値が変更されます。他の構成ファイルのディレクトリは、「{WorkMesh Installation Directory} /WorkMesh/tools/supervisord/supervisor.d/」になります。',
    ...
    xpack: {
        ...
        user: {
            ...
            auth: {
                ...
                oidc: {
                    ...
                    clockSkewHelper:
                        'ID プロバイダーと WorkMesh のわずかな時刻差を許容します。通常は 60 秒のままにしてください。',
                    ...
                    panelURL: 'WorkMesh 公開 URL',
                    panelURLHTTPWarning:
                        'HTTP は認可応答を保護しません。プロバイダーが許可した隔離されたプライベート環境でのみ使用してください。',
                    panelURLHelper:
                        'ユーザーが WorkMesh へアクセスするアドレスを入力してください。コールバック URL の生成に使用します。',
                    panelURLInvalid: 'スキーム、ホスト、およびオプションのポートのみを含むオリジンを入力します。',
                    panelURLRequired: 'WorkMesh の公開 URL を入力してください。',
                    ...
                    standardDiscoveryDesc: 'WorkMesh はエンドポイントと機能を検出し、生のドキュメントは保存しません。',
                    standardMode: '標準モード',
                ...
                saml2: {
                    ...
                    nameAttributeHelper:
                        '検証済みの SAML Assertion からこの属性を読み取り、WorkMesh のユーザー名にマッピングします。',
                    ...
                    syncLogout: '同期ログアウト',
                    syncLogoutHelper: 'WorkMesh または IdP の一方からログアウトすると、もう一方からもログアウトします。',
                    testBeforeBrowserTest: 'ブラウザのログイン テストを開始する前に、現在の設定をテストします。',
                ...
                ldap: {
                    ...
                    userSearch: 'ユーザー検索',
                    userSearchDesc:
                        'ユーザー検索ベースとフィルターを設定し、WorkMesh name として使用される LDAP 属性をマップします。',
                    ...
                    attributeMapTooltip: 'WorkMesh name フィールドから LDAP 属性への JSON マッピング。',
                    attributeMapHelper: 'WorkMesh name フィールドにマップされる LDAP 属性を設定します。',
                    advanced: '詳細設定',
                    ...
                    sync: {
                        title: 'スケジュールされた同期',
                        description:
                            'スケジュールに従って完全な LDAP ディレクトリを読み取り、インポートされた LDAP ユーザーを WorkMesh に維持します。',
                        enable: 'スケジュールを有効にする',
                    ...
                    testLoginDesc:
                        '認証のみをテストするために、AD/LDAP ユーザー資格情報をバインドします。 WorkMesh セッションは作成されません。',
        ...
        waf: {
           WAF: 'WAF',
            time: '時間',
            runtimeUnavailable: 'WAF ランタイムを利用できません。ModSecurity と CRS を含む OpenResty イメージを先にインストールしてください。',
            name: 'WAF',
        ...
        helper: {
            ...
            settingContent1: 'WorkMeshのログインページにカスタムウェルカムメッセージを設定。',
            ...
            appTitle1: '柔軟なパネル管理',
            appContent1: 'いつでもどこでもWorkMeshサーバーを簡単に管理できます。',
            ...
            vllmTitle1: '一元管理',
            vllmContent1:
                '分散した vLLM サービスを WorkMesh に統合し、インスタンス、バージョン、状態、実行ディレクトリを一元的に確認できます。',
            vllmTitle2: '高速デプロイ',
            vllmContent2:
        ...
        node: {
            ...
            nodeUpgradePackageFormatHelper:
                '`workmesh-agent_v*_*` または `workmesh-v*-linux-*` のインストールパッケージのみ対応しています',
            ...
            panelExist:
                'このノードでWorkMesh V1サービスが実行中です。追加前に移行スクリプトでV2へアップグレードしてください。',
            coreExist:
                '現在のノードはマスターノードとして有効化済みのため、直接スレーブノードとして追加できません。追加する前にまずスレーブノードにダウングレードしてください。詳細はドキュメントを参照してください。',
            agentExist:
                'このノードにworkmesh-agentが既にインストールされています。続行すると既存データを保持し、workmesh-agentサービスのみを置換します。',
            agentNotExist:
                'このノードにworkmesh-agentがインストールされていないため、ノード情報を直接編集できません。一度削除してから再度追加してください。',
            oldDataExist: 'このノードにWorkMesh V2の過去データが検出されました。以下の情報で現在の設定を上書きします:',
            ...
            fixStatus: '「systemctl status workmesh-agent.service」を実行して、サービスが起動しているか確認します。',
            ...
            uninstall: 'ノードデータを削除',
            uninstallHelper: 'ノードに関連するすべてのWorkMeshデータが削除されます。慎重に選択してください！',
        ...
        customApp: {
            ...
            imagePrefixHelper:
                '機能: イメージプレフィックスをカスタマイズし、composeファイル内のイメージフィールドを変更します。例えば、イメージプレフィックスをworkmesh/customに設定した場合、MaxKBのイメージフィールドはworkmesh/custom/maxkb:v1.10.0に変更されます',
