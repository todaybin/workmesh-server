...
const buildSshAlias = () => {
    ...
    return `workmesh-${host}-${port}-${username}-${certName}-${certID}`
        .replace(/[^a-zA-Z0-9._-]+/g, '-')
        .replace(/-+/g, '-');
};
...
const buildSetupScript = () => {
    ...
    const managedBegin = `# WorkMesh VS Code managed block: ${alias}`;
    const managedEnd = `# WorkMesh VS Code managed block end: ${alias}`;
    ...
    return [
        ...
        'if [ ! -f "$KEY_PATH" ]; then',
        '    echo "WorkMesh: private key not found: $KEY_PATH" >&2',
        '    echo "WorkMesh: download the private key and save it to this exact path, then rerun this script." >&2',
        '    exit 1',
        'fi',
        'chmod 600 "$KEY_PATH"',
        `echo "WorkMesh: SSH config ready for ${alias}"`,
    ].join('\n');
};
