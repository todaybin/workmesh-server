...
- [x] 资源诊断已拆分 cgroup `anon/file/inactive_file/swap`，可区分匿名常驻内存、可回收文件缓存和实际换出页；未新增后台采样器或主动换页策略。
...
- [x] 已实现运行时任务、转换、SSE、AI、日志读取和目录 TTL 上限，pprof 流式 gzip、RSS/PSS/cgroup 诊断、离线历史清理 CLI 与 systemd 资源 drop-in。
- [x] 定向测试、全仓编译、vet、性能微基准与发布构建通过；生产历史已按确认范围不可逆清理，新二进制和资源限制已部署，健康/就绪检查通过，空闲 RSS 约 33 MiB。
- [!] 完整 `node/api` 仍有两个既有网站代理测试与“停止创建 `nginx/proxy/root.conf`”的新行为冲突；真实认证管理流、目录首次加载和任务/SSE 峰值采样待后续维护窗口完成。
- [x] 详情见 [`2026-09-19-memory-optimization.md`](progress/2026-09-19-memory-optimization.md)。
