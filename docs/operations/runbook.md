...
先完成本机账号登录，再使用 Gateway 账号完成当前节点注册。主节点 `61.184.12.165` 和同机运行 Gateway 的次节点 `162.14.96.198` 均已使用独立 WorkMesh Server 单元部署，不能覆盖 Gateway 进程。必须为每个节点分别注册，不可由主节点代注册；跨机链路还需放行次节点通信端口。注册成功前只显示健康、登录和授权页面。
