...
| 主机监控配置与清理 | `/www/apps/1Panel/agent/app/service/monitor.go` | `GET/POST /api/v2/hosts/monitor/setting`、`POST /api/v2/hosts/monitor/setting/update`、`POST /api/v2/hosts/monitor/clean` | `node/api/host_container_cron.go` | 配置读取、间隔范围校验、清理时间记录 | 节点会话/HMAC | 请求配置与本地监控状态 | `WORKMESH_DATA_DIR/host-operational.json` 原子 rename | `node/api/host_container_cron_operational_test.go:TestHostMonitorSettingsPersistAndValidate` | `go test ./node/api -run HostMonitorSettings` | 待生产权限验证 | implemented | 监控历史由后续采集器写入 |
| 主机实时资源监控 | `/www/apps/1Panel/agent/app/service/monitor.go` | `POST /api/v2/hosts/monitor/search` | `node/api/host_container_cron.go:hostRuntimeMetrics` | POST 同左；返回 loadavg、内存、堆和协程等受限指标 | 节点会话/HMAC | `/proc`、Go runtime | 无状态实时采集 | `node/api/host_container_cron_operational_test.go` | `go test ./node/api -run HostOperational` | 待 Linux/Windows 采样核验 | implemented | Windows 缺少 loadavg 时只返回 runtime 指标 |
