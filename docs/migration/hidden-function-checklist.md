...
| [~] | Cobra/等价 CLI 管理入口 | `apps/workmesh-node/core/cmd/server/cmd/*.go` | `cmd/workmesh-server/cli.go`、`cli_test.go` | 已实现 `version`、`user-list`、`user-info`、`reset`、`listen-ip`、`app init`；`restore/update` 仍需接入签名制品校验和原子替换 |
