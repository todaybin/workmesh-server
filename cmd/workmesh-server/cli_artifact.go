...
// installSignedArtifact 校验签名制品并在数据目录内原子安装。
...
// parseArtifactOptions 解析 restore/update 制品参数并拒绝未知选项。
...
// hashArtifact 读取受限大小的普通制品文件并计算摘要。
...
// readEncodedValue 从文件或文本解析十六进制及 Base64 编码值。
...
// decodePublicKey 解析 Ed25519 公钥文本或 PEM 内容。
...
// verifyDigestSignature 验证摘要文本或摘要原始字节的 Ed25519 签名。
...
// ensureTargetWithinDataDir 确保制品目标位于数据目录且不是符号链接。
...
// atomicInstall 通过临时文件和备份完成制品原子替换。
...
// saveArtifactResult 持久化制品安装结果和回滚信息。
