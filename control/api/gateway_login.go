...
func (s *GatewayStateStore) authenticateAndBind(r *http.Request, client gateway.ProtocolClient, request gateway.LoginRequest) (gateway.Authorization, string, int, error) {
	...
	s.mu.RLock()
	nodeID, role, existingBindingID := s.status.NodeID, s.status.Role, s.auth.BindingID
	s.mu.RUnlock()
	...
	if existingBindingID != "" {
		...
		if client.Heartbeat(r.Context(), registration) == nil {
			auth.BindingID = existingBindingID
			return auth, nodeID, http.StatusOK, nil
		}
	}
	auth, err = s.registerAfterLogin(r, client, auth, nodeID, role)
	if err != nil {
...
func (s *GatewayStateStore) registerAfterLogin(r *http.Request, client gateway.ProtocolClient, auth gateway.Authorization, nodeID, role string) (gateway.Authorization, error) {
	registered, err := client.Register(r.Context(), gateway.RegisterRequest{
	...
	if err := client.Heartbeat(r.Context(), registration); err != nil {
